...
const getReturnDetail = async () => {
  try {
    ...
    ifShow.value = true
    isPlaformState.value = res.data.isPlaformState
  } catch (err) {}
}
const cancelRefundTap = async () => {
  try {
    await request(API.CancelReturnGoods, {
      afterId: item.value.afterId,
      orderId: item.value.orderId
    }, 'POST')
    uni.showToast({
