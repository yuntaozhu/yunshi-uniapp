...
onLoad((options) => {
  if (options.data) {
    currentCoupon.value = JSON.parse(options.data)
...
const getProductList = async () => {
  ...
  try {
    const res = await request(API.getCouponProducts, {
      activityId: currentCoupon.value.couponId,
      page: page.value,
